...
TEXTE = {
    ...
    's_ov_modus_h':    ('Dauerhaft sichtbar, oder nur kurz aufblenden, wenn ein Bauplan dazukommt. Im Aufblend-Betrieb genügt es, mit der Maus dorthin zu fahren, wo das Overlay steht — es kommt von selbst zurück und bleibt, solange der Zeiger darauf ist.',
                          'Permanently visible, or briefly popping up when a blueprint arrives. In pop-up mode just move the mouse to where the overlay sits — it comes back by itself and stays as long as the pointer is on it.'),
