...
| <img src="assets/symbole/22/joysticks-gruen.png" width="22" alt=""> **Joysticks** | Which stick has which number (`js1`, `js2` …), whether your bindings still point at a device that is actually plugged in — and **what is bound to which button**, searchable and filterable by device. If a stick reports under a new identifier because it sits on a different port, its old bindings can be carried over with one click (a backup is written first). Works for **any** stick, pedal or gamepad — nothing needs to be set up in advance |
