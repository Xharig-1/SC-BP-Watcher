...
- **It tells you where to get what's missing.** For most blueprints it shows which faction offers it, in which contract, from which standing, and what it pays. „I'm missing X" is half the information — „X drops at faction Y from rank Z" is all of it.
...
| 🧭 **Where it drops** | One click shows faction, contract, required standing and payout — sorted by the easiest route |
...
2. **In the background** the **`Game.log`** is read — every 3 seconds, adjustable. When the game writes `Added notification "Blueprint Received: <name>: "` on unlock, the blueprint is in the list **immediately** (🟢) and in your inventory.
