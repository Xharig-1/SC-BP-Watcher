...
| <img src="assets/symbole/22/joysticks-gruen.png" width="22" alt=""> **Joysticks & keyboard** | **What is bound to which key?** Your complete binding list in plain language — "Eject" instead of `v_eject`, in the tool's own language. Joystick, keyboard, mouse and gamepad in one searchable list, switchable between *changed by me*, *everything* and *default*. Plus: which stick has which number, and whether your bindings still point at a connected device; if a stick reports under a new identifier, its old bindings can be carried over with one click (a backup is written first). Works for **any** device — nothing needs to be set up in advance |
